...
import os, shutil
import datetime
from sample import Sample, SampleKripke, SampleCGS, consistency_checker
...
class SampleGenerator:
	...
	def generateFromLargeSampleCGS(self):
		
		generated_files = self.generateCGS(gen_from_large_sample=True)
		#generating small benchmarks from large ones
		self.generateSmallBenchmarksCGS(generated_files, self.num_models[:-1])
	...
	def generateCGS(self, gen_from_large_sample=False):
		...
		sample_folder = self.output_folder+'/CGS/' 
		os.makedirs(sample_folder)
		...
		with open(self.formula_file, 'r') as file:
			...
			sample_data = {}
			for line in file:
				print(self.formula_file)
				formula_text, propositions, players = line.split(';')
				propositions = propositions.split(',')
				propositions[-1] = propositions[-1].rstrip('\n')
				...
				formula = ATLFormula.convertTextToFormula(formula_text)
	...
	def generateSmallBenchmarksCGS(self, generated_files, num_models):
		for filename in generated_files:
			...
			for (i,j) in num_models:
				...
				new_s = SampleCGS(positive=new_positive, negative=new_negative, propositions=s.propositions, formula=s.formula)
...
def main():
	...
	if args.cgs:
		generator.generateFromLargeSampleCGS()
	else:
