...
unary_operators = ['EG', 'EF', 'EX', 'AG', 'AF', 'AX', '!']
binary_operators = ['&', '|', '->', 'EU', 'AU']
...
class CTLFormula(SimpleTree):
	...
	def prettyPrint(self, top=False):
		...
		if self.label in binary_operators:
			if self.label == 'EU':
				return lb + 'E' + lb + self.left.prettyPrint() +" U "+ self.right.prettyPrint() + rb + rb
			elif self.label == 'AU':
				return lb + 'A' + lb + self.left.prettyPrint() +" U "+ self.right.prettyPrint() + rb + rb
			else:
