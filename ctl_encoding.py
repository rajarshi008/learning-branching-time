from z3 import *
import pdb
from formulas utils.SimpleTree import CTLFormula

class SATEncodingCTL:
...
#        self.visitedPositions = set()
    ...
    """
    def encodeFormula(self, bmc=True, unsatCore=True):
                         ...
                         self.solver.assert_and_track(Implies(self.x[(i, 'G')],\
                                                               And([\
                                                                   Implies(\
                                                                             ...
                                                                             And([\
                                                                                  self.y[(i, traceIdx, timestep)] ==\
                                                                                  And(self.y[(onlyArg, traceIdx, timestep)], self.y[(i, traceIdx, tr.nextPos(timestep))])\
                                                                                  for timestep in range(tr.lengthOfTrace-1)]
                                                                                  +[self.y[(i, traceIdx, tr.lengthOfTrace-1)] ==\
                                                                                  And([self.y[(onlyArg, traceIdx, futureTimestep)] for futureTimestep in tr.futurePos(tr.lengthOfTrace-1) ])\
                                                                                  ]
                                                                                  )\
                                                                              )\
                                                                   for onlyArg in range(i)\
                                                       ...
                                                       'bmc semantics of globally operator for trace %d and depth %d' % (traceIdx, i)\
                                                       )
                         ...
                         self.solver.assert_and_track(Implies(self.x[(i, 'F')],\
                                                               And([\
                                                                   Implies(\
                                                                             ...
                                                                             And([\
                                                                                  self.y[(i, traceIdx, timestep)] ==\
                                                                                  Or(self.y[(onlyArg, traceIdx, timestep)],self.y[(i, traceIdx, tr.nextPos(timestep))])\
                                                                                  for timestep in range(tr.lengthOfTrace-1)]+
                                                                                  [self.y[(i, traceIdx, tr.lengthOfTrace-1)] ==\
                                                                                  Or([self.y[(onlyArg, traceIdx, futureTimestep)] for futureTimestep in tr.futurePos(tr.lengthOfTrace-1) ])]\
                                                                              )\
                                                                             )
                                                                   for onlyArg in range(i)\
                                                                   ])\
                                                               ),\
                                                       'bmc semantics of finally operator for trace %d and depth %d' % (traceIdx, i)\
                                                       )
                        ...
                        #until
                         self.solver.assert_and_track(Implies(self.x[(i, 'U')],\
                                                              And([ Implies(\
                                                                             ...
                                                                             And([\
                                                                                self.y[(i, traceIdx, timestep)] ==\
                                                                                Or(self.y[(rightArg, traceIdx, timestep)],
                                                                                    And(self.y[(leftArg, traceIdx, timestep)],
                                                                                        self.y[(i, traceIdx, tr.nextPos(timestep))]\
                                                                                        )\
                                                                                  )\
                                                                                for timestep in range(tr.lengthOfTrace-1)\
                                                                                  ]+\
                                                                                ...
                                                                                Or([\
                                                                                    And(\
                                                                                        [self.y[(leftArg, traceIdx, futurePos)] for futurePos in tr.futurePos(tr.lengthOfTrace-1)[0:qIndex]]+\
                                                                                        [self.y[(rightArg, traceIdx, tr.futurePos(tr.lengthOfTrace-1)[qIndex])]]\
                                                                                        )\
                                                                                    for qIndex in range(len(tr.futurePos(tr.lengthOfTrace-1)))\
                                                                                    ])]
                                                                                 )\
                                                                             )\
                                                                    for leftArg in range(i) for rightArg in range(i) ])),\
                                                        'bmc semantics of Until operator for trace %d and depth %d'%(traceIdx, i))
